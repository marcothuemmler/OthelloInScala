...
class Player(name: String, value: Int, var board: Board) {

  def this(value: Int, game: Board) = this(f"Player$value", value, game)
  ...
  def getMoves(x: Int, y: Int): ((Int, Int), List[(Int, Int)]) = {
    ((x, y), (for {
      i <- -1 to 1
      j <- -1 to 1 if i != 0 || j != 0
    } yield check(x, y, (i, j))).filter(_ != (-1, -1)).toList)
  }
  ...
  def set(x: Int, y: Int): Boolean = {
    ...
    if (valid.nonEmpty) {
      valid.keys.foreach(board.flipLine((x, y),_ , value))
  ...
  def setByOpp(x: Int, y: Int): Boolean = {
    board.grid(x)(y).isSet && board.grid(x)(y).value != this.value
  }
