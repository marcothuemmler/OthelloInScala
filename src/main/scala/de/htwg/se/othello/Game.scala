...
class Game {
  ...
  def checkMoves(x: Int, y: Int, p: Player): ((Int, Int), List[(Int, Int)]) = {
    ...
    list += check(x, y, p, -1,0)
    list += check(x, y, p, 1, 0)
    list += check(x, y, p, 0,1)
    list += check(x, y, p, 0, -1)
    list += check(x, y, p, -1, -1)
    list += check(x, y, p, 1, -1)
    list += check(x, y, p, -1, 1)
    list += check(x, y, p, 1, 1)
