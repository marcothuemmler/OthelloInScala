...
class MoveSelectorSpec extends WordSpec with Matchers {
  ...
  "An EasyBot" should {
    "select a bad move using the search algorithm if the board size is 8x8" in {
      val moveSelector = new EasyBot(ctrl)
      ctrl.createBoard(8)
      ctrl.boardController.board = ctrl.boardController.board.flipLine((4, 3), (2, 3), 1)
      ctrl.boardController.board.flipLine((2, 4), (2, 4), 2)
      ctrl.boardController.board = ctrl.boardController.board.flipLine((4, 4), (4, 5), 1)
      ctrl.boardController.board = ctrl.boardController.board.flipLine((3, 4), (5, 4), 2)
      val selection = moveSelector.selection
