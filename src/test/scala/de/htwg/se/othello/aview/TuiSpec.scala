...
class TuiSpec extends WordSpec with Matchers {
  ...
  "A Tui " should {
    ...
    "create a new game on input n" in {
      ...
      controller.getBoard should be((new CreateBoardStrategy).createNewBoard(8))
    }
    ...
    "not set a square and not flip any disk on input a12" in {
      ...
      controller.getBoard should equal(board)
    }
    val ctrl = new Controller
    ctrl.setupPlayers("2")
    val t = new Tui(ctrl)
    var changedBoard = ctrl.getBoard
    "undo a step on input z" in {
      ctrl.newGame
      t.processInputLine("c4")
      changedBoard = ctrl.getBoard
      t.processInputLine("z")
      ctrl.getBoard should equal((new CreateBoardStrategy).createNewBoard(8))
    }
